...
try {
    ...
    foreach ($data['items'] as $item) {
        ...
        if (!mysqli_query($conn, $update_sql)) {
            ...
            throw new Exception(json_encode(['error' => 'Insufficient stock for product ID: ' . $product_id]));
        }

        if (mysqli_affected_rows($conn) === 0) {
            //  throw new Exception(json_encode(['error' => 'Failed to update product quantity for ID: ' . $product_id]));
            throw new Exception('Failed to update product quantity for ID: ' . $product_id);
        }
    }

    // $select = mysqli_query($conn, "SELECT * FROM placed_orders WHERE order_id = '$order_id' ");
    // $row = mysqli_fetch_assoc($select);

    // if ($row['total_price'] === $row['payed_amount']) {
    //     $updateq = mysqli_query($conn, "UPDATE placed_orders SET payment_status = 1 WHERE order_id = '$order_id'");
